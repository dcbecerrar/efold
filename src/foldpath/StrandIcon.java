...
//		g2d.fillOval((length/2)-2, (length/2)-2, 4, 4);
	...
	public void drawStrand(Graphics2D g, double start, double end, Dir d, double xOffset, double yOffset, Color borderColor, String label){
		AffineTransform at = new AffineTransform();
		if(d == Dir.PARA)
			at.rotate(Math.PI, 4*scale, 10*scale);
	...
	public void drawSheet(Graphics g, int x, int y){		
		...
		for(int i = 0; i<template.size(); i++){
			...
			//			g.drawImage(strandImage(start, end, d, scale), xOffset, 0, null);
			if(i==0){
				label = Landscape_tfolder.clusterLabelShort(template, pairing);
			}else{
				label = "";
			}

			drawStrand((Graphics2D) g, start, end, d, xOffset+x, y, borderColor,label);
